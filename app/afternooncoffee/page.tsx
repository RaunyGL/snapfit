...
export default async function AfternoonCoffee() {
  const afternooncoffees = await db.afternoonCoffee.findMany({
    include: {
      food: true,
  }});
      ...
      <div>
        <ScrollArea className="overflow-y-scroll [&::-webkit-scrollbar]:hidden max-h-[500px] w-11/12 mx-auto mt-6 rounded-md border bg-white drop-shadow-lg">
          <div className="mt-6 flex justify-end mr-3">
            <Link href="./insert-afternooncoffee">
          ...
          <div className="p-4">
            ...
            <ul>
              {afternooncoffees.map((afternooncoffee) => (
                <li className="text-base">{afternooncoffee.food.name}
                <li className="text-sm">{afternooncoffee.food.servingSize} - {afternooncoffee.food.calories} kcal</li>
                <EditDialog />
