...
export default async function Lunch() {
  const lunchs = await db.lunch.findMany({
    include: {
      food: true,
  }});
      ...
      <div>
        <ScrollArea className="overflow-y-scroll [&::-webkit-scrollbar]:hidden max-h-[500px] w-11/12 mx-auto mt-6 rounded-md border bg-white drop-shadow-lg">
          ...
          <div className="p-4">
            ...
            <ul>
              {lunchs.map((lunch) => (
                <li className="text-base">{lunch.food.name}
                <li className="text-sm">{lunch.food.servingSize} - {lunch.food.calories} kcal</li>
                <EditDialog />
